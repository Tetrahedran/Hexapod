...
/* USING SECTION
	...
	- send one char / character:		uart_putc( (unsigned char)c );		( hex: 0x.. / bin: 0b........ || 2 = 0x02 / 0b00000010 )
	- send an int / Number from var:	sendIntUart( int n );	
	- send a static text:				uart_puts_P( "static text: " );		( ...be careful with the high case 'P' - its necessary )
	- send a char array:				sendString( createdCharArray );		( ...could be used also with a reference to the object: &createdCharArray )
...
#define UART_RX_BUFFER_SIZE 64 //usual 32
#endif
...
#define UART_TX_BUFFER_SIZE 64 //usual 32
#endif
...
/**
** Additional functions (normally not included in Fleury-Lib)
*/

void sendString (char stringChar[]);
void sendIntUart(int n);

void UART_Flush( void ); //from datasheet Atmega644PA, 21.8.7./p.235 Flushing the Receive Buffer
